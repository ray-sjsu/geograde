...
export default function Reviews({ locationId, setAverageRating, setReviewCount }) {
  ...
  const saveReview = async () => {
    ...
    try {
      ...
      const locationDoc = await getDoc(locationDocRef); // Fix: Use getDoc for a single document
  ...
  return (
    <div className="w-full max-w-4xl space-y-4 overflow-auto mt-5">
      ...
      {reviews.map(({ id, rating, noiseLevel, textContent, userEmail, date, additionalFeedback }) => (
